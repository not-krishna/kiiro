      ...
      <div className="max-w-7xl mx-auto space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[#E8E1D5] pb-8">
          <div className="max-w-2xl space-y-3">
            <h2 className="font-display text-fluid-3xl font-normal">Upcoming programming</h2>
            <p className="text-fluid-base text-[#6E635B] font-light leading-relaxed">
              Public sessions for individuals and small groups. Group and corporate programmes follow a separate enquiry path.
            </p>
          </div>
          <Link
            href="/weekly-events"
            className="text-[15px] font-medium text-[#2B231F] border-b-2 border-[#2B231F] pb-1 hover:text-[#C2593F] hover:border-[#C2593F]"
          >
            View all events
          </Link>
        </div>
            ...
            {events.slice(0, 4).map((event) => (
              <li key={event.id} className="py-8 grid md:grid-cols-12 gap-4 md:gap-8 items-start">
                <div className="md:col-span-3 text-sm text-[#6E635B] space-y-1">
                  <p className="text-[#C2593F] font-medium">{formatEventDate(event.date) || 'Date to be confirmed'}</p>
                ...
                <div className="md:col-span-6 space-y-2">
                  <h3 className="font-display text-2xl text-[#2B231F]">{event.title}</h3>
